...
int lower_exception_entry(trapframe *tf)
{
  ...
  if (ec == 0b010101)
  {
    ...
    case 10:
      ...
      break;
    case 11:
      tf->x[0] = sys_open((const char *)tf->x[0], tf->x[1]);
      break;
    case 12:
      tf->x[0] = sys_close(tf->x[0]);
      break;
    case 13:
      tf->x[0] = sys_write(tf->x[0], (const void *)tf->x[1], tf->x[2]);
      break;
    case 14:
      tf->x[0] = sys_read(tf->x[0], (void *)tf->x[1], tf->x[2]);
      break;
    case 15:
      tf->x[0] = sys_mkdir((const char *)tf->x[0], tf->x[1]);
      break;
    case 16:
      tf->x[0] = sys_mount((const char *)tf->x[0], (const char *)tf->x[1], (const char *)tf->x[2], tf->x[3], (const void *)tf->x[4]);
      break;
    case 17:
